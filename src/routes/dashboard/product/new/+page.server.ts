...
export const actions = {
  createInfo: async ({ request }) => {
    ...
    if (!name) {
      return fail(400, { name, price, qty, images, errorMsg: "Name value missing" });
    }
    if (!price || isNaN(+price) || +price < 0) {
      return fail(400, {
        name, price, qty, images,
    ...
    if (!images) {
      return fail(400, { name, price, qty, images, errorMsg: "images value missing" });
    } else {
    ...
    const { data, error } = await supabase
      .from("Products")
      .insert([{ name, price: +price, qty: +qty, img: listImages }])
      .select();
